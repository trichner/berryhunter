...
const hasToken = QueryParameters.get().has(BasicConfig.VALUE_PARAMETERS.TOKEN);
const token = QueryParameters.get().getString(BasicConfig.VALUE_PARAMETERS.TOKEN, '');
let domReady = false;
let _isOpen = false;
...
BackendValidTokenEvent.subscribe( () => {
    ...
    Preloading.renderPartial(require('../assets/console.html'), onDomReady);
});
...
function onDomReady() {
    ...
    QueryParameters.get().tryGetStringArray(BasicConfig.VALUE_PARAMETERS.START_COMMANDS, (commands => {
