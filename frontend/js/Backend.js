...
define(['Game', 'Utils', 'Constants', 'Develop', 'vendor/flatbuffers', 'schema_common', 'schema_server', 'schema_client'],
	function (Game, Utils, Constants, Develop, flatbuffers, DeathioApi) {
		const Backend = {
			setup: function () {
			...
			receive: function (event) {
				if (!Game.two.playing && gameStarted) {
					return;
				}
			...
			receiveSnapshot: function (snapshot) {
				...
				snapshot.entities.forEach(function (entity) {
					if (entity.id === snapshot.playerId) {
						if (Game.started) {
							Game.player.character.setPosition(entity.x, entity.y);
						} else {
							Game.createPlayer(entity.id, entity.x, entity.y);
						}
						if (Develop.isActive()) {
							Game.player.character.updateAABB(entity.aabb);
						}
					} else {
